...
<template>
  <div class="py-8 px-4 sm:px-6 lg:px-8">
    <div class="max-w-6xl mx-auto space-y-8">
      <!-- Header -->
      <div>
        <h1 class="text-2xl font-bold text-[var(--color-text)]">管理面板</h1>
        <p class="mt-1 text-[var(--color-text-secondary)]">管理和分析您的所有短链接</p>
      </div>
      ...
      <section v-if="linkStore.popularLinks.length > 0">
        <BaseCard>
          <h3 class="text-lg font-semibold text-[var(--color-text)] mb-4">热门链接</h3>
          <div class="space-y-3">
            <div
              v-for="(link, index) in linkStore.popularLinks"
              :key="link.id"
              class="flex items-center gap-4 p-3 bg-[var(--color-bg-secondary)] rounded-lg"
            >
              <!-- Rank -->
              <div
                :class="[
              ...
              <div class="flex-shrink-0 text-right">
                ...
                <p class="text-xs text-[var(--color-text-muted)]">点击</p>
              </div>
            </div>
          </div>
        </BaseCard>
